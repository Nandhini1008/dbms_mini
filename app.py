...
def signup():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute("INSERT INTO Users (name, password, role) VALUES (%s, %s, %s)", (name, hashed_password, role))
...
def equipment_search():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute("SELECT * FROM Equipment WHERE name LIKE %s", (f"%{name}%",))
